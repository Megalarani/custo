...
if(media.name.includes('.jpeg') || (media.name.includes('.jpg')) || (media.name.includes('.png')) ){
  uploadBytes(storageRef, media).then((snapshot) => {getDownloadURL(storageRef).then((url) => {
...
if (gallerytype === null) {
  ui = (
    <>
     { galdata!== null &&
                galdata.images.length !== 0 && galdata.images.map((data,index) =>{
 return (
  <div key={index}>
...
{ galdata!== null &&
                galdata.videos.length !== 0 && galdata.videos.map((data,index) =>{
 return (
  <div key={index} className="video-gal" >
   <iframe src={data.vid} style={{ width:"500", height:"500"}} ></iframe>;
  </div>
  );
})}
    </>
 ...
 return (
  ...
  ui = (
    <>
     { galdata!== null &&
                galdata.videos.length !== 0 && galdata.videos.map((data) =>{
 return (
    ...
    <div className="upload-container">
      <div className="border-container">
        ...
        <p>Drag and drop files here, or 
          <a href="#" id="file-browser"><label for="pic">
  <span> <i className="fa fa-upload mx-2"> browse </i> </span>
  <input type="file" id="pic" onChange={handleUpload} accept="file_extension|video/*|image/*|media_type"  />

</label></a> your computer.</p>
<button type ="submit" className="btn my-3"> <i className="fa fa-upload mx-2 "></i> Click to upload a data</button>
</form>
      </div>
    </div>
  </div>
...
<ul class="switcher">
  <li data-cat=".all" onClick={ Allmedia }>All</li>
  
  <li data-cat=".pictures" onClick={ Allimages }>Pictures</li>
  <li data-cat=".Videos" onClick={ Allvideos }>Videos</li>
 
</ul>

<div class="gallery mx-2">
{ui}
